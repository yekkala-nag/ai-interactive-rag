...
export default function ModelRoutingTab() {
  ...
  const rc = r.tier.includes('Fast') ? '#10b981' : r.tier.includes('Balanced') ? '#F5A623' : '#ef4444';
  return (
      ...
      <Container size="wide">
        ...
        {sub === 'scoring' && (<Stack gap={6}><Card style={{ padding: 'var(--ds-space-5)', background: 'var(--ds-color-bg-canvas)' }}><Stack gap={3}>
          ...
          <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
            ...
            <tbody>{SCORE_DIMS.map((d, i) => (<tr key={i} style={{ borderBottom: '1px solid var(--ds-color-border-subtle)' }}><td style={{ padding: '8px', color: 'white', fontWeight: 'bold' }}>{d.dim}</td><td style={{ padding: '8px', color: '#10b981' }}>{d.low}</td><td style={{ padding: '8px', color: '#F5A623' }}>{d.mid}</td><td style={{ padding: '8px', color: '#ef4444' }}>{d.high}</td></tr>))}</tbody></table></div>
          <Grid columns={{ base: '1fr', md: '1fr 1fr 1fr' }} gap="var(--ds-space-2)">{TIERS.map((t, i) => (<Card key={i} style={{ padding: '10px', background: 'var(--ds-color-bg-surface)', borderLeft: '3px solid #10b981' }}><div style={{ fontSize: '12px', color: 'white', fontWeight: 'bold' }}>{t.tier} · {t.range}</div><div style={{ fontSize: '11px', color: 'var(--ds-color-text-secondary)' }}>{t.use}</div></Card>))}</Grid>
          <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
            ...
            <tbody>{EXPERIMENTS.map((e, i) => (<tr key={i} style={{ borderBottom: '1px solid var(--ds-color-border-subtle)' }}><td style={{ padding: '8px', color: 'white' }}>{e.q}</td><td style={{ padding: '8px', textAlign: 'center', color: '#10b981', fontFamily: 'monospace', fontWeight: 'bold' }}>{e.adap}</td><td style={{ padding: '8px', textAlign: 'center', color: 'var(--ds-color-text-tertiary)', fontFamily: 'monospace' }}>{e.full}</td><td style={{ padding: '8px', textAlign: 'center', color: '#38BDF8', fontWeight: 'bold' }}>{e.save}</td><td style={{ padding: '8px', color: 'var(--ds-color-text-secondary)' }}>{e.why}</td></tr>))}</tbody></table></div>
        </Stack></Card></Stack>)}
