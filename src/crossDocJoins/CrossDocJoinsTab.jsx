    ...
    <div style={{ paddingBottom: 'var(--ds-space-12)' }}>
      ...
      <Container size="wide">
        ...
        {sub === 'align' && (<Stack gap={6}><Card style={{ padding: 'var(--ds-space-5)', background: 'var(--ds-color-bg-canvas)' }}><Stack gap={3}>
          <Grid columns={{ base: '1fr', md: '1fr 1fr 1fr 1fr' }} gap="var(--ds-space-2)">{ALIGN_STEPS.map(s2 => (<Card key={s2.step} style={{ padding: '12px', background: 'var(--ds-color-bg-surface)', borderLeft: '3px solid #5EC4C8' }}><div style={{ fontSize: '12px', color: '#3A9B9F', fontWeight: 'bold' }}>{s2.step}. {s2.name}</div><div style={{ fontSize: '11px', color: 'var(--ds-color-text-secondary)' }}>{s2.detail}</div></Card>))}</Grid>
          <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
            ...
            <tbody>{MAPPING_TABLE.map((m, i) => (<tr key={i} style={{ borderBottom: '1px solid var(--ds-color-border-subtle)' }}><td style={{ padding: '8px', color: 'white', fontFamily: 'monospace' }}>{m.contractA}</td><td style={{ padding: '8px', color: 'white', fontFamily: 'monospace' }}>{m.contractB}</td><td style={{ padding: '8px', color: '#3A9B9F', fontFamily: 'monospace' }}>{m.unified}</td><td style={{ padding: '8px', textAlign: 'center', color: m.conf < 0.85 ? '#F5A623' : '#5EC4C8', fontWeight: 'bold' }}>{m.conf}</td><td style={{ padding: '8px', textAlign: 'center', color: 'var(--ds-color-text-secondary)' }}>{m.status}</td></tr>))}</tbody></table></div>
        </Stack></Card></Stack>)}
        {sub === 'sim' && (<Stack gap={6}><Card style={{ padding: 'var(--ds-space-5)', background: 'var(--ds-color-bg-canvas)' }}><Stack gap={4}>
          ...
          <Card style={{ padding: '14px', background: 'var(--ds-color-bg-surface)', borderLeft: '4px solid #5EC4C8' }}>
            ...
            <div style={{ fontSize: '13px', color: '#3A9B9F', fontWeight: 'bold', marginTop: '8px' }}>corpus total = {u.total}</div>
            <div style={{ fontSize: '11px', color: '#3A9B9F', fontFamily: 'monospace', marginTop: '4px' }}>{u.sql}</div>
