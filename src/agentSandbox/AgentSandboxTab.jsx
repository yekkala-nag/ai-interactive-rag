    ...
    <div style={{ paddingBottom: 'var(--ds-space-12)' }}>
      ...
      <Container size="wide">
        ...
        {sub === 'tiers' && (<Stack gap={6}><Card style={{ padding: 'var(--ds-space-5)', background: 'var(--ds-color-bg-canvas)' }}><Stack gap={3}>
          <Grid columns={{ base: '1fr', md: '1fr 1fr 1fr 1fr' }} gap="var(--ds-space-2)">{SANDBOX_TIERS.map((t, i) => (<Card key={i} style={{ padding: '12px', background: 'var(--ds-color-bg-surface)', borderLeft: `3px solid ${i === 3 ? '#ef4444' : i === 0 ? '#5EC4C8' : '#5EC4C8'}` }}><div style={{ fontSize: '12px', color: 'white', fontWeight: 'bold' }}>{t.tier}</div><div style={{ fontSize: '11px', color: '#3A9B9F' }}>✓ {t.allows}</div><div style={{ fontSize: '11px', color: '#ef4444' }}>✕ {t.blocks}</div><div style={{ fontSize: '11px', color: 'var(--ds-color-text-secondary)' }}>{t.use}</div></Card>))}</Grid>
          <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
            ...
            <tbody>{TOOL_RISKS.map((r, i) => (<tr key={i} style={{ borderBottom: '1px solid var(--ds-color-border-subtle)' }}><td style={{ padding: '8px', color: 'white', fontFamily: 'monospace' }}>{r.tool}</td><td style={{ padding: '8px', textAlign: 'center', color: r.risk === 'Critical' ? '#ef4444' : r.risk === 'High' ? '#F5A623' : '#5EC4C8', fontWeight: 'bold' }}>{r.risk}</td><td style={{ padding: '8px', color: 'var(--ds-color-text-secondary)' }}>{r.control}</td></tr>))}</tbody></table></div>
        </Stack></Card></Stack>)}
