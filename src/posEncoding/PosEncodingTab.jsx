    ...
    <div style={{ paddingBottom: 'var(--ds-space-12)' }}>
      ...
      <Container size="wide">
        ...
        {sub === 'pipeline' && (<Stack gap={6}><Card style={{ padding: 'var(--ds-space-5)', background: 'var(--ds-color-bg-canvas)' }}><Stack gap={3}>
          <Grid columns={{ base: '1fr', md: '1fr 1fr 1fr 1fr 1fr' }} gap="var(--ds-space-2)">{PE_PIPELINE.map((s, i) => (<Card key={i} style={{ padding: '12px', background: 'var(--ds-color-bg-surface)', borderLeft: `3px solid ${i === 3 ? '#ef4444' : i === 4 ? '#10b981' : '#38BDF8'}` }}><div style={{ fontSize: '12px', color: 'white', fontWeight: 'bold' }}>{s.step}</div><div style={{ fontSize: '11px', color: 'var(--ds-color-text-secondary)' }}>{s.detail}</div><div style={{ fontSize: '11px', color: '#38BDF8', fontFamily: 'monospace' }}>{s.math}</div></Card>))}</Grid>
          <Grid columns={{ base: '1fr', md: '1fr 1fr' }} gap="var(--ds-space-2)">
            ...
            <Card style={{ padding: '12px', background: 'var(--ds-color-bg-surface)' }}>
              <strong style={{ fontSize: '12px', color: '#38BDF8' }}>Three encodings:</strong>
        ...
        {sub === 'sim' && (<Stack gap={6}><Card style={{ padding: 'var(--ds-space-5)', background: 'var(--ds-color-bg-canvas)' }}><Stack gap={4}>
          ...
          <Grid columns={{ base: '1fr', md: '1fr 1fr' }} gap="var(--ds-space-4)">
            ...
            <Card style={{ padding: '14px', background: 'var(--ds-color-bg-surface)', borderLeft: '4px solid #F5A623' }}>
              ...
              <div style={{ fontSize: '12px', color: '#10b981', marginTop: '6px' }}>with p: {d.withPE}</div>
              <div style={{ fontSize: '12px', color: '#38BDF8', marginTop: '6px' }}>{lag} → {lagInfo.captures}</div>
