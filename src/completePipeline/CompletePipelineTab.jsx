...
const TONE = { ok: '#10b981', bad: '#ef4444', warn: '#F5A623', info: 'var(--ds-color-text-secondary)' };
              ...
              <Grid columns={{ base: '1fr', md: '1fr 1fr' }} gap="var(--ds-space-4}">
                <Card style={{ padding: '14px', background: 'var(--ds-color-bg-surface)', animation: 'fadeIn 0.4s ease' }}>
                  ...
                  <Grid columns={{ base: '1fr', md: '1fr 1fr 1fr' }} gap="var(--ds-space-2)" style={{ marginTop: '8px' }}>
                    {[['Precision@' + k, m.precision, m.precision >= 0.5 ? '#10b981' : '#ef4444'], ['Recall', m.recall, m.recall >= 0.5 ? '#10b981' : '#ef4444'], ['Faithfulness', m.faithfulness, m.evaluated ? (m.faithfulness >= 0.85 ? '#10b981' : '#F5A623') : '#64748b']].map(([l, v, c]) => (
                ...
                <Card style={{ padding: '14px', background: 'var(--ds-color-bg-surface)', animation: 'fadeIn 0.4s ease' }}>
                  ...
                  <Stack gap={2} style={{ marginTop: '8px' }}>
                    {result.chunks.map((c, i) => {
                      ...
                      return (
                        <div key={c.id} style={{ fontSize: '11px', fontFamily: 'monospace', padding: '6px 8px', borderRadius: '6px', background: rel ? 'rgba(16,185,129,0.1)' : 'rgba(239,68,68,0.08)', borderLeft: `3px solid ${rel ? '#10b981' : '#ef4444'}` }}>
                          <span style={{ color: 'var(--ds-color-text-tertiary)' }}>#{i + 1} [{c.id}]</span>{' '}
                          <span style={{ color: rel ? '#10b981' : '#f87171' }}>{rel ? 'RELEVANT' : 'distractor'}</span>{' '}
