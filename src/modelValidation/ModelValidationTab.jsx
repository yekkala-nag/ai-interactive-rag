...
export default function ModelValidationTab() {
  ...
  const tc = t.tier === 'High' ? '#ef4444' : t.tier === 'Medium' ? '#F5A623' : '#2AB5B0';
      ...
      <Container size="wide">
        ...
        {sub === 'pillars' && (<Stack gap={6}><Card style={{ padding: 'var(--ds-space-5)', background: 'var(--ds-color-bg-canvas)' }}><Stack gap={3}>
          ...
          <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
            ...
            <tbody>{FIVE_BREAKS.map((b, i) => (<tr key={i} style={{ borderBottom: '1px solid var(--ds-color-border-subtle)' }}><td style={{ padding: '8px', color: '#ef4444', fontWeight: 'bold' }}>{b.property}</td><td style={{ padding: '8px', color: 'var(--ds-color-text-secondary)' }}>{b.why}</td><td style={{ padding: '8px', color: '#17837F' }}>{b.consequence}</td></tr>))}</tbody></table></div>
          <Grid columns={{ base: '1fr', md: '1fr 1fr' }} gap="var(--ds-space-2)">{DIMENSION_SETS.map((d, i) => (<Card key={i} style={{ padding: '12px', background: 'var(--ds-color-bg-surface)', borderLeft: '3px solid #FF8A6B' }}><div style={{ fontSize: '12px', color: 'white', fontWeight: 'bold' }}>{d.set}</div><div style={{ fontSize: '11px', color: '#17837F', fontFamily: 'monospace' }}>{d.dims}</div><div style={{ fontSize: '11px', color: '#F5A623' }}>⚠ {d.trap}</div></Card>))}</Grid>
        </Stack></Card></Stack>)}
        {sub === 'sim' && (<Stack gap={6}><Card style={{ padding: 'var(--ds-space-5)', background: 'var(--ds-color-bg-canvas)' }}><Stack gap={4}>
          ...
          <Grid columns={{ base: '1fr', md: '1fr 1fr' }} gap="var(--ds-space-4)">
            ...
            <Card style={{ padding: '14px', background: 'var(--ds-color-bg-surface)', borderLeft: `4px solid ${tc}` }}>
              ...
              <div style={{ fontSize: '11px', color: '#17837F', marginTop: '4px' }}>First move: {t.first}</div>
          ...
          <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
            ...
            <tbody>{TIER_TABLE.map((r, i) => (<tr key={i} style={{ borderBottom: '1px solid var(--ds-color-border-subtle)' }}><td style={{ padding: '8px', textAlign: 'center', color: r.tier === 'High' ? '#ef4444' : r.tier === 'Medium' ? '#F5A623' : '#2AB5B0', fontWeight: 'bold' }}>{r.tier}</td><td style={{ padding: '8px', color: 'var(--ds-color-text-secondary)' }}>{r.evidence}</td><td style={{ padding: '8px', textAlign: 'center', color: 'var(--ds-color-text-secondary)' }}>{r.monitor}</td></tr>))}</tbody></table></div>
        </Stack></Card></Stack>)}
        {sub === 'code' && (<Stack gap={6}><Card style={{ padding: 'var(--ds-space-5)', background: 'var(--ds-color-bg-canvas)' }}><Stack gap={4}>
          ...
          <Grid columns={{ base: '1fr', md: '1fr 1fr' }} gap="var(--ds-space-2">
            {[...COMPONENT_MAP.slice(0, 4).map(c => ({ t: c.comp, d: `${c.controls} controls — ${c.breaks}`, col: c.controls === 'Vendor' ? '#ef4444' : '#2AB5B0' })), ...ROBUSTNESS_TESTS.map(r => ({ t: `Perturb: ${r.perturb}`, d: `${r.how} — ${r.star}`, col: '#F5A623' })), ...JUDGE_TESTS.map(j => ({ t: `Judge: ${j.test}`, d: j.why, col: '#FF8A6B' })), ...MONITOR_INDICATORS.map(m => ({ t: m.ind, d: `catches: ${m.catches}`, col: '#2AB5B0' }))].map((x, i) => (
