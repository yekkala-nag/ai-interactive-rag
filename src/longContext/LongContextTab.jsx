      ...
      <Container size="wide">
        ...
        {sub === 'failures' && (<Stack gap={6}><Card style={{ padding: 'var(--ds-space-5)', background: 'var(--ds-color-bg-canvas)' }}><Stack gap={3}>
          ...
          {FAILURE_MODES.map((f, i) => (<Card key={i} style={{ padding: '12px', background: 'var(--ds-color-bg-surface)', borderLeft: '4px solid #ef4444' }}>
            ...
            <Grid columns={{ base: '1fr', md: '1fr 1fr' }} gap="var(--ds-space-2)" style={{ fontSize: '11px', marginTop: '6px' }}>
              ...
              <div><span style={{ color: '#3A9B9F', fontWeight: 'bold' }}>Fix: </span><span style={{ color: 'var(--ds-color-text-secondary)' }}>{f.fix}</span></div></Grid></Card>))}
        </Stack></Card></Stack>)}
        {sub === 'sim' && (<Stack gap={6}><Card style={{ padding: 'var(--ds-space-5)', background: 'var(--ds-color-bg-canvas)' }}><Stack gap={4}>
          ...
          <Grid columns={{ base: '1fr', md: '1fr 1fr' }} gap="var(--ds-space-4)">
            ...
            <Card style={{ padding: '14px', background: 'var(--ds-color-bg-surface)', borderLeft: `4px solid ${plan.fits ? '#5EC4C8' : '#ef4444'}` }}>
              <div style={{ fontSize: '12px', color: 'white' }}>Usable: {(plan.usable / 1000).toFixed(0)}k · Fits: {plan.fits ? 'yes' : 'NO'}</div>
              <div style={{ fontSize: '12px', color: plan.depthRisk.startsWith('HIGH') ? '#ef4444' : '#5EC4C8', fontWeight: 'bold' }}>Depth risk: {plan.depthRisk}</div>
              <div style={{ fontSize: '12px', color: '#3A9B9F', fontWeight: 'bold', marginTop: '6px' }}>{plan.strategy}</div>
        ...
        {sub === 'code' && (<Stack gap={6}><Card style={{ padding: 'var(--ds-space-5)', background: 'var(--ds-color-bg-canvas)' }}><Stack gap={4}>
          ...
          <Grid columns={{ base: '1fr', md: '1fr 1fr 1fr 1fr' }} gap="var(--ds-space-2)">{COMPRESSION_LADDER.map((l, i) => (<Card key={i} style={{ padding: '10px', background: 'var(--ds-color-bg-surface)', borderLeft: '3px solid #5EC4C8' }}><div style={{ fontSize: '12px', color: 'white', fontWeight: 'bold' }}>{l.level}</div><div style={{ fontSize: '11px', color: '#3A9B9F', fontFamily: 'monospace' }}>{l.ratio}</div><div style={{ fontSize: '11px', color: 'var(--ds-color-text-secondary)' }}>{l.use}</div><div style={{ fontSize: '11px', color: '#F5A623' }}>risk: {l.risk}</div></Card>))}</Grid>
