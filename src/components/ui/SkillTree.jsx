...
function nodeColor(id) {
  if (isProven(id)) return { fill: 'rgba(16,185,129,0.16)', stroke: '#10b981', text: '#e2e8f0' };
  if (isClaimed(id)) return { fill: 'rgba(245,166,35,0.12)', stroke: '#F5A623', text: '#e2e8f0' };
  const s = getMasteryScore(id);
  if (s > 0) return { fill: 'rgba(56,189,248,0.12)', stroke: '#38BDF8', text: '#e2e8f0' };
  return { fill: 'rgba(255,255,255,0.03)', stroke: '#475569', text: '#94a3b8' };
}
  ...
  return (
    <div>
        ...
        <svg viewBox={`0 0 860 ${H}`} style={{ minWidth: '620px', width: '100%', height: 'auto', display: 'block' }} role="img" aria-label={`Skill tree for ${mod.title}`}>
          ...
          {tabs.map(t => {
            ...
            return getPrereqIds(t.id).map(p => {
              ...
              const mx = (x1 + x2) / 2;
              return <path key={`${p}-${t.id}`} d={`M${x1},${y1} C${mx},${y1} ${mx},${y2} ${x2},${y2}`} fill="none" stroke={isMastered(p) ? '#10b981' : '#334155'} strokeWidth={isMastered(p) ? 2 : 1.2} opacity={isMastered(p) ? 0.9 : 0.6} />;
            });
          })}
          {/* nodes */}
          {tabs.map(t => {
      ...
      <div style={{ display: 'flex', gap: '14px', flexWrap: 'wrap', marginTop: '8px', fontSize: '0.7rem', color: 'var(--ds-color-text-tertiary)' }}>
        <span><span style={{ color: '#10b981' }}>●</span> proven</span>
        <span><span style={{ color: '#F5A623' }}>●</span> claimed</span>
        <span><span style={{ color: '#38BDF8' }}>●</span> in progress</span>
        <span><span style={{ color: '#475569' }}>●</span> untouched</span>
