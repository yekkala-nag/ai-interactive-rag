  ...
  return (
    <div style={{ background: '#ffffff', borderRadius: 8, border: '1px solid #e0dcd4', padding: '1.25rem' }}>
      <div style={{ marginBottom: '1.25rem' }}>
        <div style={{ fontFamily: 'Syne, sans-serif', fontSize: '0.65rem', fontWeight: 700, color: '#C47A6A', letterSpacing: '0.1em', textTransform: 'uppercase', marginBottom: '0.3rem' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', marginBottom: '1.5rem' }}>
        <StatCard label="Micro Recall (Before LLM)" value={`${microRecallBefore}%`} sub="299 / 415 native rows" color="#5EC4C8" />
        <StatCard label="Micro Precision (Before LLM)" value={`${microPrecisionBefore}%`} sub="299 / 1481 candidates" color="#F0A89A" />
        <StatCard label="Micro Precision (After LLM)" value="87.0%" sub="+67% precision gain" color="#16a34a" />
        <StatCard label="Page Accuracy (±1)" value="100%" sub="Across all matched rows" color="#9B89C4" />
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '1.25rem' }}>
            ...
            {BENCHMARK_FIXTURES.map(f => (
              <button
                ...
                style={{
                  ...
                  borderLeft: selectedFixture.name === f.name ? '3px solid #5EC4C8' : '3px solid transparent',
                  cursor: 'pointer',
                }}
              >
        ...
        <div style={{ border: '1px solid #e2e8f0', borderRadius: 6, padding: '1rem', background: '#ffffff' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1rem' }}>
            ...
            <ProgressBar label="Precision After LLM Validation" value={selectedFixture.precisionAfter} color="#5EC4C8" />
            <ProgressBar label="Recall Before LLM" value={selectedFixture.recallBefore} color="#5EC4C8" />
