...
export default function EnterpriseGradeTab() {
  ...
  const rc = r.score >= 85 ? '#10b981' : r.score >= 60 ? '#F5A623' : '#ef4444';
  return (
    <div style={{ paddingBottom: 'var(--ds-space-12)' }}>
      ...
      <Container size="wide">
        ...
        {sub === 'plays' && (<Stack gap={6}><Card style={{ padding: 'var(--ds-space-5)', background: 'var(--ds-color-bg-canvas)' }}><Stack gap={3}>
          <Grid columns={{ base: '1fr', md: '1fr 1fr 1fr' }} gap="var(--ds-space-2)">{VALUE_PLAYS.map((v, i) => (<Card key={i} style={{ padding: '12px', background: 'var(--ds-color-bg-surface)', borderLeft: '3px solid #10b981' }}><div style={{ fontSize: '12px', color: 'white', fontWeight: 'bold' }}>{v.play}</div><div style={{ fontSize: '11px', color: 'var(--ds-color-text-secondary)' }}>{v.ex}</div><div style={{ fontSize: '11px', color: '#10b981' }}>{v.why}</div></Card>))}</Grid>
          <Grid columns={{ base: '1fr', md: '1fr 1fr' }} gap="var(--ds-space-2)">
            <Card style={{ padding: '12px', background: 'var(--ds-color-bg-surface)' }}>
              <strong style={{ fontSize: '12px', color: '#38BDF8' }}>Data diligence (CRISP-DM heart):</strong>
