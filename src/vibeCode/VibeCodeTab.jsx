...
export default function VibeCodeTab() {
  ...
  const rc = r.net >= 0 ? '#10b981' : '#ef4444';
  return (
      ...
      <Container size="wide">
        ...
        {sub === 'case' && (<Stack gap={6}><Card style={{ padding: 'var(--ds-space-5)', background: 'var(--ds-color-bg-canvas)' }}><Stack gap={3}>
          <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
            ...
            <tbody>{UNIT_CASE.map((u, i) => (<tr key={i} style={{ borderBottom: '1px solid var(--ds-color-border-subtle)' }}><td style={{ padding: '8px', color: 'white', fontWeight: 'bold' }}>{u.line}</td><td style={{ padding: '8px', textAlign: 'center', color: i === 2 || i === 3 ? '#ef4444' : '#10b981', fontFamily: 'monospace', fontWeight: 'bold' }}>{u.value}</td><td style={{ padding: '8px', color: 'var(--ds-color-text-secondary)' }}>{u.note}</td></tr>))}</tbody></table></div>
          <Grid columns={{ base: '1fr', md: '1fr 1fr 1fr' }} gap="var(--ds-space-2)">{FAILURE_MODES.map((f, i) => (<Card key={i} style={{ padding: '12px', background: 'var(--ds-color-bg-surface)', borderLeft: '3px solid #ef4444' }}><div style={{ fontSize: '12px', color: '#ef4444', fontWeight: 'bold' }}>{f.mode}</div><div style={{ fontSize: '11px', color: 'white' }}>{f.detail}</div><div style={{ fontSize: '11px', color: '#10b981' }}>→ {f.fix}</div></Card>))}</Grid>
        </Stack></Card></Stack>)}
