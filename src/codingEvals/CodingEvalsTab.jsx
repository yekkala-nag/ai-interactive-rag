...
export default function CodingEvalsTab() {
  ...
  const rc = r.verdict.startsWith('SHIP') ? '#10b981' : r.verdict.startsWith('GATE') ? '#F5A623' : '#ef4444';
    ...
    <div style={{ paddingBottom: 'var(--ds-space-12)' }}>
      ...
      <Container size="wide">
        ...
        {sub === 'contracts' && (<Stack gap={6}><Card style={{ padding: 'var(--ds-space-5)', background: 'var(--ds-color-bg-canvas)' }}><Stack gap={3}>
          ...
          <div style={{ fontSize: '12px', color: coverage === 100 ? '#10b981' : coverage >= 60 ? '#F5A623' : '#ef4444', fontWeight: 'bold' }}>
          ...
          <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
            ...
            <tbody>{SIX_LAYERS.map((l, i) => (<tr key={i} style={{ borderBottom: '1px solid var(--ds-color-border-subtle)' }}><td style={{ padding: '8px', color: 'white', fontWeight: 'bold' }}>{l.layer}</td><td style={{ padding: '8px', color: 'var(--ds-color-text-secondary)' }}>{l.asks}</td><td style={{ padding: '8px', color: '#38BDF8', fontFamily: 'monospace' }}>{l.metric}</td></tr>))}</tbody></table></div>
        </Stack></Card></Stack>)}
        {sub === 'sim' && (<Stack gap={6}><Card style={{ padding: 'var(--ds-space-5)', background: 'var(--ds-color-bg-canvas)' }}><Stack gap={4}>
          ...
          <Grid columns={{ base: '1fr', md: '1fr 1fr' }} gap="var(--ds-space-2)">
            <Card style={{ padding: '12px', background: 'var(--ds-color-bg-surface)' }}>
              <strong style={{ fontSize: '12px', color: '#38BDF8' }}>Statistics rules</strong>
            ...
            <Card style={{ padding: '12px', background: 'var(--ds-color-bg-surface)' }}>
              <strong style={{ fontSize: '12px', color: '#c9a84c' }}>Open-ended bounds</strong>
