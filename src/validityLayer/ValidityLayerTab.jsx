      ...
      <Container size="wide">
        ...
        {sub === 'states' && (<Stack gap={6}><Card style={{ padding: 'var(--ds-space-5)', background: 'var(--ds-color-bg-canvas)' }}><Stack gap={3}>
          ...
          <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '11px' }}>
            ...
            <tbody>{EXPERIMENT_TABLE.map((e, i) => (<tr key={i} style={{ borderBottom: '1px solid var(--ds-color-border-subtle)' }}><td style={{ padding: '8px', color: 'white', fontWeight: 'bold' }}>{e.exp}</td><td style={{ padding: '8px', color: 'var(--ds-color-text-secondary)' }}>{e.tests}</td><td style={{ padding: '8px', textAlign: 'center', color: '#ef4444' }}>{e.base}</td><td style={{ padding: '8px', textAlign: 'center', color: '#17837F' }}>{e.aware}</td></tr>))}</tbody></table></div>
          <Grid columns={{ base: '1fr', md: '1fr 1fr 1fr 1fr' }} gap="var(--ds-space-2)">{WHEN_TO_BUILD.map((w, i) => (<Card key={i} style={{ padding: '10px', background: 'var(--ds-color-bg-surface)', borderLeft: `3px solid ${w.verdict.startsWith('Build') ? '#2AB5B0' : '#64748b'}` }}><div style={{ fontSize: '11px', color: 'white' }}>{w.use}</div><div style={{ fontSize: '11px', fontWeight: 'bold', color: w.verdict.startsWith('Build') ? '#2AB5B0' : 'var(--ds-color-text-tertiary)' }}>{w.verdict}</div></Card>))}</Grid>
        </Stack></Card></Stack>)}
        {sub === 'sim' && (<Stack gap={6}><Card style={{ padding: 'var(--ds-space-5)', background: 'var(--ds-color-bg-canvas)' }}><Stack gap={4}>
          ...
          <Grid columns={{ base: '1fr', md: '1fr 1fr' }} gap="var(--ds-space-4)">
            ...
            <Card style={{ padding: '14px', background: 'var(--ds-color-bg-surface)', borderLeft: '4px solid #2AB5B0' }}>
              ...
              <div style={{ fontSize: '12px', color: '#17837F', marginTop: '4px' }}>Aware: {r.aware.steps} steps, 0 doomed → {r.aware.completes ? 'completes' : 'FAILS'}</div>
